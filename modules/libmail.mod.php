...
function Attach( $filename, $filetype = "", $disposition = "inline", $basename = "" )
{
...
function _build_attachement()
{
	...
	/*
	 * If filebasename is set, $filename must be an attachment stream instead of a file
	 */
	if($this->filebasename != "") {
		...
		for( $i=0; $i < count( $this->aattach); $i++ ) {
			...
			$ata[$k++] = $subhdr;
			$ata[$k++] = chunk_split(base64_encode($filename)); // format $data using RFC 2045 semantics
		}
	/*
	 * attach all files
	 */
