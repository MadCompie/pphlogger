<?php
 /* ---------------------------------------------------------
    ...
    $Id: admin2.php 1759 2003-08-18 19:15:45Z cvs_iezzi $
...
switch(@$action) {
	case 'orderby':
		if ($useraccount_ord == $order) {
			...
			mysqli_query($GLOBALS['mysql_link'], "UPDATE ".PPHL_TBL_SETTINGS." SET value = '$new_desc' WHERE setting = 'useraccount_ord_desc'");
		} else {
			mysqli_query($GLOBALS['mysql_link'], "UPDATE ".PPHL_TBL_SETTINGS." SET value = '$order' WHERE setting = 'useraccount_ord'");
			mysqli_query($GLOBALS['mysql_link'], "UPDATE ".PPHL_TBL_SETTINGS." SET value = 'false' WHERE setting = 'useraccount_ord_desc'");
		}
