...
switch(@$action) {
	case 'orderby':
		if ($useraccount_ord == $order) {
			...
			mysqli_query($link, "UPDATE ".PPHL_TBL_SETTINGS." SET value = '$new_desc' WHERE setting = 'useraccount_ord_desc'");
		} else {
			mysqli_query($link, "UPDATE ".PPHL_TBL_SETTINGS." SET value = '$order' WHERE setting = 'useraccount_ord'");
			mysqli_query($link, "UPDATE ".PPHL_TBL_SETTINGS." SET value = 'false' WHERE setting = 'useraccount_ord_desc'");
		}
